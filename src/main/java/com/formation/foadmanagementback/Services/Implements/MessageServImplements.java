...
import com.formation.foadmanagementback.Mappers.IMessageMapper;
...
public class MessageServImplements implements IMessageService {
    ...
    public MessageDTO sendMessage(MessageCreateDTO dto) {
        ...
        Message message = IMessageMapper.toEntity(dto, expediteur, destinataire);
        return IMessageMapper.toDTO(iMessagesRepository.save(message));
    }

    @Override
    public MessageDTO getByUuid(UUID uuid) {
        return iMessagesRepository.findByUuid(uuid)
            .map(IMessageMapper::toDTO)
